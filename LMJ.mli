...
and binop =
  ...
  | OpSubAssign (** Binary operator [-=]. *)
  | OpMulAssign (** Binary operator [*=]. *)
  | OpDivAssign (** Binary operator [/=]. *)
  | OpModAssign (** Binary operator [%=]. *)
...
and instruction =
  ...
  | IFor of instruction * expression * instruction * instruction (** [IFor (e1, e2, e3, i)] represents the instruction [for (e1; e2; e3) i]. *)
